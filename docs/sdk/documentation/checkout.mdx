...
the response as part of the checkout object. You can still get them by using the
method below.
...
Get the shipping rate for the selected products on the checkout.

```js
async function getShipping(PRODUCT_ID) => {
  const sdk = getSDK();
  const shipping = await sdk.shopCart.shipping.getShippingByProductId(PRODUCT_ID);
  return shipping;
});
```

As a response, you will get an array of the selected shipping classes for that
specific product:
    ...
    "shippingCountry": [
      {
        ...
        "id": "b924a960-9584-4b58-8da0-ebd48e878d8d",
        "amount": "50.00",
        "country": "NO",
        "deletedAt": null,
        "currencyCode": "EUR"
